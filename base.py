...
def set_privacy_and_embed():
    ...
    for id in get_list_of_vid_ids():
        print('Editing {}'.format(id))
        uri = 'https://api.vimeo.com/videos/{}'.format(id)
        client = get_client()
        ...
        response = client.patch(uri, json=json_data)
        if response.status_code == 200:
            print('Edit successful')
        else:
            print('Edit fail: {}'.format(str(response.status_code)))
...
def add_whitelist_domain():
    ...
    for id in get_list_of_vid_ids():
        print('Editing {}'.format(id))
        uri = 'https://api.vimeo.com/videos/{}/privacy/domains/{}'.format(id, DOMAIN)
        client = get_client()
        response = client.put(uri)
        if response.status_code == 204:
            print('Edit successful')
        else:
            print('Edit fail: {}. Reason: {}'.format(str(response.status_code)), response.reason)
...
def init_funcs():
    funcs = {
        ...
        "--set-privacy": set_privacy_and_embed,
        "--set-domain": add_whitelist_domain,
    }
    return funcs
